...
async def chat_with_bot(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # ===============================
        crop_context = "No crop data available."
        ...
        # ===============================
        # 6. LIVE SENSOR PACKAGE
        # ===============================
        live_stats = "No live sensor data available."

        if latest_reading:
            weather_info = (
                f"Temp: {weather_record.temperature}°C, "
                f"Rain: {weather_record.precipitation_probability}%"
                if weather_record
                else "No weather data"
            )

            live_stats = (
                f"Soil Moisture: {latest_reading.soil_moisture * 100:.1f}%, "
        ...
        system_instruction = (
            ...
            "Tone: Practical, concise, professional. "
            "Give irrigation advice based on these specific numbers."
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="AquaBot is temporarily unavailable."
        )